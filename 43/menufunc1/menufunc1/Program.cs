...
namespace menufunc1
{
    class Program
    {
        ...
        static void stampa_menu()
        {
            ...
            Console.WriteLine("F) Ordina in maniera crescente");
            Console.WriteLine("G) Ordina in maniera decrescente");
        ...
        static void ordinacresc()
        {
            int i;
            int j;
            int n = numeri.Length;
            for (i=0; i<=n-2; i++) // mi scorro tutti gli elementi dell'array dal primo (0) al penultimo (n-2)
            {
                // Ora per ogni elmento numeri[i] devo confrontarlo con tutti quelli alla sua detra, cioè quelli che vanno da i+1 all'ultimo n-1
                for(j=i+1; j<=n-1; j++) // uso il for per scorrere con un'altra variabile tutti i valori da i+1 a salire fino ad n-1
                {
                    //adesso faccio il confronto
                    if (numeri[i]>numeri[j]) // verifico se c'è una condizione che non deve esserci nell'ordinamento crescente
                    {
                        // Se è così devo invertire i numeri
                        int x = numeri[i];
